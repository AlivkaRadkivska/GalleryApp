...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!token)
        return next(new AppError('Будь ласка, спочатку авторизуйтеся'), 401);

    const decoded = await promisify(jwt.verify)(token, process.env.JWT_KEY);
    const user = await User.findById(decoded.id);

    if (!user)
        return next(new AppError('Такого користувача не знайдено', 404));
...
exports.login = catchAsync(async (req, res, next) => {
    ...
    if (!email || !password)
        return next(new AppError('Електрона пошта та пароль обов\'язкові', 400));

    const item = await User.findOne({ email }).select('+password');
    if (!item)
        return next(new AppError('Користувач з такою поштою не зареєстрований', 401));
